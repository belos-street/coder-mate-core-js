...
import { CPP_TOKENIZER_SPEC } from './spec'
import type {
  GrammarState,
  ParserContext,
  Token,
...
export const createInitialContext = (): ParserContext =>
  createInitialContextCore(CPP_TOKENIZER_SPEC.initialState)
...
export const splitTokenByLineBreak = (
  ...
  splitTokenByLineBreakCore(
    text,
    scope,
    CPP_TOKENIZER_SPEC.fallbackScope,
    startLine,
    startCol
  )
...
export const matchToken = (
  ...
  matchTokenCore(code, context, CPP_TOKENIZER_SPEC)

export const parse = (code: string): TokenStream =>
  parseCore(code, CPP_TOKENIZER_SPEC)
