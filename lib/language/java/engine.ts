...
import { JAVA_TOKENIZER_SPEC } from './spec'
import type {
  GrammarState,
  ParserContext,
  Token,
...
export const createInitialContext = (): ParserContext =>
  createInitialContextCore(JAVA_TOKENIZER_SPEC.initialState)
...
export const splitTokenByLineBreak = (
  ...
  splitTokenByLineBreakCore(
    text,
    scope,
    JAVA_TOKENIZER_SPEC.fallbackScope,
    startLine,
    startCol
  )
...
export const matchToken = (
  ...
  matchTokenCore(code, context, JAVA_TOKENIZER_SPEC)

export const parse = (code: string): TokenStream =>
  parseCore(code, JAVA_TOKENIZER_SPEC)
