...
import { YAML_TOKENIZER_SPEC } from './spec'
import type {
  GrammarState,
  ParserContext,
  Token,
...
export const createInitialContext = (): ParserContext =>
  createInitialContextCore(YAML_TOKENIZER_SPEC.initialState)
...
export const splitTokenByLineBreak = (
  ...
  splitTokenByLineBreakCore(
    text,
    scope,
    YAML_TOKENIZER_SPEC.fallbackScope,
    startLine,
    startCol
  )
...
export const matchToken = (
  ...
  matchTokenCore(code, context, YAML_TOKENIZER_SPEC)

export const parse = (code: string): TokenStream =>
  parseCore(code, YAML_TOKENIZER_SPEC)
