...
import { CSHARP_TOKENIZER_SPEC } from './spec'
import type {
  GrammarState,
  ParserContext,
  Token,
...
export const createInitialContext = (): ParserContext =>
  createInitialContextCore(CSHARP_TOKENIZER_SPEC.initialState)
...
export const splitTokenByLineBreak = (
  ...
  splitTokenByLineBreakCore(
    text,
    scope,
    CSHARP_TOKENIZER_SPEC.fallbackScope,
    startLine,
    startCol
  )
...
export const matchToken = (
  ...
  matchTokenCore(code, context, CSHARP_TOKENIZER_SPEC)

export const parse = (code: string): TokenStream =>
  parseCore(code, CSHARP_TOKENIZER_SPEC)
