...
import { RUST_TOKENIZER_SPEC } from './spec'
import type {
  GrammarState,
  ParserContext,
  Token,
...
export const createInitialContext = (): ParserContext =>
  createInitialContextCore(RUST_TOKENIZER_SPEC.initialState)
...
export const splitTokenByLineBreak = (
  ...
  splitTokenByLineBreakCore(
    text,
    scope,
    RUST_TOKENIZER_SPEC.fallbackScope,
    startLine,
    startCol
  )
...
export const matchToken = (
  ...
  matchTokenCore(code, context, RUST_TOKENIZER_SPEC)

export const parse = (code: string): TokenStream =>
  parseCore(code, RUST_TOKENIZER_SPEC)
